...
const parseAction = (description) => {
    if (!description) return { badge: 'Aktivitas', color: 'bg-base-200 text-base-content border-base-300', text: 'Tidak ada deskripsi.' };

    const d = description.toLowerCase();

    if (description === 'created') return { badge: 'Dibuat', color: 'bg-success/15 text-success border-success/30', text: 'Menambahkan data baru ke sistem.' };
    if (description === 'updated') return { badge: 'Diedit', color: 'bg-warning/15 text-warning border-warning/30', text: 'Memperbarui data yang sudah ada.' };
    if (description === 'deleted') return { badge: 'Dihapus', color: 'bg-error/15 text-error border-error/30', text: 'Menghapus data dari sistem.' };

    if (d.includes('import')) return { badge: 'Import', color: 'bg-info/15 text-info border-info/30', text: description };
    if (d.includes('buat') || d.includes('tambah')) return { badge: 'Dibuat', color: 'bg-success/15 text-success border-success/30', text: description };
    if (d.includes('edit') || d.includes('update')) return { badge: 'Diedit', color: 'bg-warning/15 text-warning border-warning/30', text: description };
    if (d.includes('hapus')) return { badge: 'Dihapus', color: 'bg-error/15 text-error border-error/30', text: description };

    return { badge: 'Aktivitas', color: 'bg-base-200 text-base-content border-base-300', text: description };
};
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="space-y-6 max-w-7xl mx-auto pb-12 px-4 sm:px-0">
                ...
                {/* --- HEADER --- */}
                <div className="flex items-center gap-4 bg-base-100 p-5 sm:p-6 rounded-2xl shadow-sm border border-base-200">
                    <div className="bg-primary/10 p-3 rounded-full flex-shrink-0">
                        <History className="h-6 w-6 text-primary" />
                    </div>
                    <div>
                        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-base-content">Riwayat Sistem</h1>
                    ...
                    <div className="relative space-y-3">
                        {activities.data.map((log) => {
                            ...
                            return (
                                <div
                                    key={log.id}
                                    className="bg-base-100 rounded-2xl border border-base-200 p-4 sm:p-5 hover:shadow-md transition-all duration-200"
                                >
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 mb-3">
                                        ...
                                        <span className="inline-flex items-center gap-1.5 font-bold text-sm text-base-content">
                                            <Database className="w-4 h-4 text-base-content/40" />
                                            {log.log_name || 'Umum'}
                                        </span>
                                        {log.subject_type && (
                                            <span className="text-[11px] font-semibold text-base-content/50 bg-base-200 px-2 py-0.5 rounded-md">
                                                {log.subject_type}#{log.subject_id}
                                            </span>
                                        )}
                                        <span className="inline-flex items-center gap-1.5 text-xs text-base-content/50 ml-auto">
                                    ...
                                    {/* Deskripsi */}
                                    <p className="text-sm text-base-content/80 mb-3">{act.text}</p>
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
                                        ...
                                        {hasDetails ? (
                                            <details className="group w-full sm:w-auto sm:min-w-[18rem] bg-base-200/50 rounded-xl border border-base-200 [&_summary::-webkit-details-marker]:hidden">
                                                <summary className="flex items-center justify-between p-2.5 cursor-pointer hover:bg-base-200 transition-colors rounded-xl">
                                                ...
                                                <div className="p-3 border-t border-base-200 text-xs">
                                            ...
                                            <span className="inline-flex items-center gap-1.5 text-xs text-base-content/30 italic">
...
const TONE = {
    success: { box: 'border-success/30', head: 'text-success border-success/10', strike: 'decoration-success/50' },
    error: { box: 'border-error/30', head: 'text-error border-error/10', strike: 'decoration-error/50' },
};
...
function DetailBlock({ title, tone, data, strike = false }) {
    ...
    return (
        <div className={`bg-base-100 p-3 rounded-xl border ${c.box} shadow-sm`}>
            ...
            <ul className="space-y-2">
                {Object.entries(data).map(([key, value]) => (
                    <li
                        key={key}
                        className="grid grid-cols-1 sm:grid-cols-[140px_1fr] items-start gap-0.5 sm:gap-3 border-b border-base-200/50 pb-2 last:border-0 last:pb-0"
                    >
                        <span className="text-base-content/60 font-medium break-words">{formatKey(key)}</span>
                        <span className={`font-semibold text-base-content break-words ${strike ? `line-through ${c.strike}` : ''}`}>
                            {formatValue(value)}
                        </span>
                    </li>
                ))}
