          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className=" space-y-6">
              <div className='flex flex-row justify-between items-center'>
                ...
                {/* start breadcrumbs */}
                <Breadcrumbs items={[{ label: 'Home', href: '/' }, { label: 'Daerah' }, { label: 'Fokus' }]} />
                {/* end breadcrumbs */}
              ...
              <Card>
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                  {/* Button Tambah User */}
                  {hasPermission('create fokus daerah') && (
                    <Button asChild className="rounded-lg">
                      <Link href={route('admin.daerah.fokus.create')}>
              ...
              <Card>
                ...
                <div className="md:hidden flex flex-col gap-4">
                  ...
                  {focus.data.map((fokus) => (
                    <div key={fokus.id} className="border rounded-xl p-4 bg-background shadow-sm">
                      ...
                      <div className="flex gap-2 mt-4">
                        {hasPermission('edit fokus daerah') && (
                          <Button asChild size="sm" variant="outline" className="border-amber-500 text-amber-600 hover:bg-amber-50 hover:text-amber-700 dark:border-amber-500/60 dark:text-amber-400 dark:hover:bg-amber-950/40">
                            <Link href={route('admin.daerah.fokus.edit', fokus)}>Edit</Link>
                          </Button>
                        )}
                      </div>
                    </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {focus.data.map((fokus, index) => (
                        <TableRow key={fokus.id}>
                          <TableCell>{index + 1}</TableCell>
                          <TableCell>{fokus.name}</TableCell>
                          <TableCell>{formatDate(fokus.created_at)}</TableCell>
                          <TableCell>
                            {getStatusBadge(fokus.status)}
                          </TableCell>
                          <TableCell>
