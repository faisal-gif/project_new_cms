...
function Index({ pakets, filters }) {
    ...
    const INDEX_ROUTE = route('admin.kopi-times.paket.index');
                        ...
                        <div className="space-y-6">
                            ...
                            <Card>
                                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                                    {hasPermission(['create paket kopi-times']) && (
                                        <Link href={route('admin.kopi-times.paket.create')} className="btn btn-primary rounded-lg">
                                ...
                                <div className="md:hidden flex flex-col gap-4">
                                    {pakets.data.map((paket) => (
                                        <div key={paket.id} className="border rounded-xl p-4 bg-base-100 shadow-sm">
                                            ...
                                            {hasPermission(['edit paket kopi-times']) && (
                                                <div className="flex gap-2 mt-4">
                                                    <Link href={route('admin.kopi-times.paket.edit', paket.id)} className="btn btn-sm btn-warning btn-outline w-full">Edit Paket</Link>
                                                </div>
                                            )}
                                        </div>
                                    ))}
                                                    ...
                                                    <td>
                                                        {hasPermission(['edit paket kopi-times']) && (
                                                            <div className="flex justify-end gap-2">
                                                                <Link href={route('admin.kopi-times.paket.edit', paket.id)} className="btn btn-sm btn-warning btn-outline">Edit</Link>
                                                            </div>
                                                        )}
                                                    </td>
                                                </tr>
