...
use App\Http\Requests\WriterKtRequest;
...
class WriterKtController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('Admin/Kopi_Times/Writer/Index', [
            'writers' => $writers,
    ...
    {
        $paket = PaketBerita::where('status', 1)->where('type', '4')->get();
        return Inertia::render('Admin/Kopi_Times/Writer/Create', [
            'paket' => $paket
        ]);
    }
     ...
     */
    public function store(WriterKtRequest $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.kopi-times.writer.index')->with('success', 'Penulis Kopi Times berhasil ditambahkan.');
        } catch (\Exception $e) {
            DB::rollback();
